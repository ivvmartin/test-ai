...
function parseZDDS(text: string): VATArticle[] {
  ...
  for (const part of parts) {
    ...
    articles.push({
      ...
      content: `Чл. ${articleNumber}. ${content}`,
    });
  }
...
/**
 * Parse ППЗДДС (Regulations) articles
 * Format: "§ X" or "§ X."
 */
...
// Statistics
console.log("ЗДДС Articles:", ZDDS_CONTENT.length);
console.log("ППЗДДС Articles:", PPZDDS_CONTENT.length);
...
async function main() {
  ...
  if (args.length !== 2) {
    console.error("❌ Error: Please provide paths to both PDF files");
    console.log("\nUsage:");
  ...
  if (!fs.existsSync(ppzddsPdfPath)) {
    console.error(`❌ Error: ППЗДДС PDF not found: ${ppzddsPdfPath}`);
    process.exit(1);
  }
  ...
  console.log(`   ЗДДС Articles: ${zddsArticles.length}`);
  console.log(`   ППЗДДС Articles: ${ppzddsArticles.length}`);
